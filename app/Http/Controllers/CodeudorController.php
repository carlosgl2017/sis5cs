...
class CodeudorController extends Controller
{
     ...
     public function index(Request $request)
    {
    	 if ($request)
        {
             ...
             $codeudores=DB::table('persona as p')
             ->join('codeudor as c','c.id_persona','=','p.id_persona')
             ->select('p.id_persona','p.ci','p.nombre','p.ap_paterno','p.ap_materno','p.ap_casada','p.fec_nac','p.genero','p.celular','p.dependientes','p.estado_civil','p.id_profesion','c.id_codeudor as id_codeudor')
            ->where('p.nombre','ILIKE','%'.$query.'%')
       
            ->orderBy('id_codeudor','desc')
            ->paginate(7);
            return view('codeudor.crud.index',["codeudores"=>$codeudores,"searchText"=>$query]);
    ...
    public function create()
    {
        ...
        return view('codeudor.crud.create')->with(compact('personas','clientes'));//listado
    ...
    public function store(CodeudorFormRequest $request)
    {
      ...
      $codeudor->id_persona=$request->input('id_persona');
      $codeudor->id_cliente=$request->input('id_cliente');
      $codeudor->save(); //metodo se encarga de ejecutar un insert sobre la tabla

      return redirect('/codeudor/crud');
    }
     ...
     public function edit($id)
    {
      ...
      return view('codeudor.crud.edit')->with(compact('persona','profesion')); //formulario de registro
    }
