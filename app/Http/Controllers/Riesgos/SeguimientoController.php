...
class SeguimientoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $usuarios = User::All();
        $areas = Area::All();
        return view('riesgos.seguimiento.index')->with(compact('seguimiento', 'usuarios', 'areas'));
    }
            ...
            if ($existe_registros) //si no existe registros
            {
                if ($seguimiento->last()->usuario_destino == Auth::user()->id_users) {
                    ...
                    if ($seguimiento->last()->id_users == Auth::user()->id_users) {
                        ...
                        return view('riesgos.seguimiento.create');
                    }
                } else {
            ...
            if ($seguimiento->last()->id_users == Auth::user()->id_users) {
                if ($seguimiento->last()->id_seguimiento == $id) 
                {
                    if (empty($seguimiento->last()->fecha_fin)) {
                        return view('riesgos.seguimiento.fin')->with(compact('segui'));
                    } else {
    ...
    public function reporte()
    {
        ...
        $pdf = PDF2::loadView('riesgos.seguimiento.reporte', ['seguimiento' => $seguimiento, 'usuarios' => $usuarios, 'areas' => $areas, 'date' => $date,'primer_registro'=>$primer_registro,'ultimo'=>$ultimo]);
