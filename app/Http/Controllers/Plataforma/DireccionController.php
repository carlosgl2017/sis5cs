...
public function index(Request $request)
{
  ...
  if(session('id_persona')==null)
   ...
   else
   {
    ...
    ->get();
    return view('plataforma.direccion.index')->with(compact('dir'));
 ...
 if(session('id_persona')==null)
   ...
   else
   {
    ...
    if($if_exist>0)
    {
     ...
     return redirect('plataforma/direccion/');
   }
   else
   {
    $tipo=TipoVivienda::all();
    return view('plataforma.direccion.create')
    ->with(compact('tipo'));  
...
public function store(DireccionFormRequest $request)
{
 ...
 return redirect('plataforma/direccion')->with(compact('notification'));
}
...
public function edit($id)
{
 ...
 $tipo_casa=TipoVivienda::All();
    return view('plataforma.direccion.edit')->with(compact('dir','tipo_casa')); //formulario de registro
  }
