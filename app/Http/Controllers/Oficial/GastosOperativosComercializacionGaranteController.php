...
class GastosOperativosComercializacionGaranteController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (session('id_persona_garante') == null) {
            ...
            $gastos = GastosOperativosComercializacion::where('id_persona', session('id_persona_garante'))->where('id_credito', session('id_credito'))->get();
            return view('oficial.a_garantes.gastos_operativos.index')->with(compact('gastos'));
        }
    ...
    public function create()
    {
        if (session('id_persona_garante') == null) {
            alert()->info('Info', 'Seleccione un garante')->showConfirmButton();
            return redirect('oficial/a_garantes/garante/');
        } else {
            $if_exist = GastosOperativosComercializacion::where('id_persona', session('id_persona_garante'))->where('id_credito', session('id_credito'))->count();
            if ($if_exist > 0) {
                alert()->info('Info', 'Ya registro los datos de gastos Operativos.')->showConfirmButton();
                return redirect('oficial/a_garantes/gastos_operativos/');
            } else {

                return view('oficial.a_garantes.gastos_operativos.create');
            }
    ...
    public function edit($id)
    {
        ...
        return view('oficial.a_garantes.gastos_operativos.edit')->with(compact('gastos')); //formulario de registro
    }
