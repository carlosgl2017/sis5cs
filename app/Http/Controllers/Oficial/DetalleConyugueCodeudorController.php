...
class DetalleConyugueCodeudorController extends Controller
{
    ...
    {
        $id = Conyugue::where('id_persona', session('id_persona_codeudor'))->firstOrFail()->conyugue;
        $detalle = DetallePersona::where('id_persona', $id)->get();
        return view('oficial.a_codeudores.detalle_conyugue.index')->with(compact('detalle'));
    }
    ...
    public function create()
    {
        if (session('id_persona_codeudor') == null) {
            ...
            $id = Conyugue::where('id_persona', session('id_persona_codeudor'))->count();
            if ($id > 0) {
                ...
                if (Detallepersona::where('id_persona', $conyugue)->count() >= 1) {
                    ...
                    return redirect('oficial/a_codeudores/detalle_conyugue/');
                } else {
                    return view('oficial.a_codeudores.detalle_conyugue.create');
                }
            } else {
                alert()->info('Info', 'Registre Conyugue Primero')->showConfirmButton();
                return redirect('oficial/a_codeudores/conyugue/create');
            }
    ...
    {
        //id_conyugue
        $id_conyugue = Conyugue::where('id_persona', session('id_persona_codeudor'))->firstOrFail();
        ...
        return redirect('oficial/a_codeudores/detalle_conyugue/')->with(compact('notification'));

    }
    public function edit($id)
    {
        $detalle = DetallePersona::find($id);
        return view('oficial.a_codeudores.detalle_conyugue.edit')->with(compact('detalle')); //formulario de registro
    }
