    ...
    public function index(Request $request)
    {
        ...
        if (!$credito->isEmpty()) {
            ...
            $garantias = DB::table('credito')
                ...
                ->where('credito.id_persona', $credito[0]->id_persona)
                ->get();
        } else {
        ...
        $if_exist_deposito = DepositoBancario::where('id_persona', $id)->where('id_credito', $idc)->count();
...
//total suma de pasivos y patrimonio
        ...
        $if_exist_ventas = Ventas::where('id_persona', $id)->count();
        $ventas = Ventas::where('id_persona', $id)->get();
...
//total suma de pasivos y patrimonio
        ...
        $inf_prestamos = DB::table('prestamo_bancario')
            ...
            ->where('id_persona', session('id_persona'))
            ->get();
        return view('oficial.formulario.index1')
            ->with(compact('direccion'
                , 'persona'
                , 'personas'
                , 'datos_empresa'
