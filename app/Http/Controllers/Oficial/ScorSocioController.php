...
class ScorSocioController extends Controller
{
    ...
    public function scor()
    {
        ...
        $exi_garantia = Garantia::where('id_credito', session('id_credito'))->where('id_persona', session('id_persona'))->get()->isEmpty();
        $exi_reporte = ReporteBuro::where('id_persona', $id)->where('id_credito', session('id_credito'))->get()->isEmpty();
        $exi_efectivo = EfectivoCaja::where('id_persona', $id)->where('id_credito', session('id_credito'))->get()->isEmpty();
        $exi_capacidad = CapacidadPago::where('id_persona', $id)->where('id_credito', session('id_credito'))->get()->isEmpty();
        if ($exi_direccion) {
        ...
        //tabla credito
        $credito = Credito::where('id_credito', $idc)->firstOrFail();
        $capacidad_pago12 = CapacidadPago::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail();
        ...
        $total_experiencia_crediticia = PuntajeRepository::experiencia_cre_ultimo($this->experienciaCrediticia($id, $idc)) + PuntajeRepository::experiencia_cre_ultimo($this->penultima_experienciaCrediticia($id, $idc));
        //calculo total de c1
        ...
        $contador_c1 = ContadorC1Repository::contador_c1($direccion->id_tipo_vivienda, $tiempo_residencia, $this->tiempoTrabajoActividad($id), $this->tiempoTrabajoEmpresa($id), $this->experienciaCrediticia($id, $idc), $this->penultima_experienciaCrediticia($id, $idc));
        if ($contador_c1 == 0) {
            $porcentaje_c1 = 0;
        } else {
        ...
        $endeudamiento_actual = $this->endeudamientoActual($id, $idc); //con la funcion para controlar division entre cero
        $monto_solicitado = Credito::where('id_credito', $idc)->firstOrFail()->monto_solicitado;
        $endeudamiento_con_este_credito = $this->endeudamientoConEsteCredito($id, $idc);
        ...
        $c3_sum_eval = PuntajeRepository::cobertura_cuota($this->coverturaCuotaIngreso($id, $idc)) + PuntajeRepository::gasto_anterior($this->gastosIngresosAnterior($id,$idc)) + PuntajeRepository::gasto_actual($this->gastosIngresosActual($id,$idc));
        $contador_c3 = ContadorC3Repository::contador_c3($this->coverturaCuotaIngreso($id, $idc), $this->gastosIngresosAnterior($id,$idc), $this->gastosIngresosActual($id,$idc));
        if ($contador_c3 == 0) {
            $porcentaje_c3 = 0;
        } else {
        ...
        //--------Calculo c4-----------
        $ingresos_fijos_mensuales = $this->totalIngresos($id, $idc);
        $ingresos_variables_mensuales = $this->ingresos_variables_mensuales($id, $idc);
        ...
        if ($this->garantiaC5_1($id, $idc) == 0) {
            $tipoGarantia1 = "";
        } else {
            $tipoGarantia1 = TipoGarantia::where('id_tipo_garantia', $this->garantiaC5_1($id, $idc))->firstOrFail()->tipo_garantia;
        }

        if ($this->garantiaC5_2($id, $idc) == 0) {
            $tipoGarantia2 = "";
        } else {
            $tipoGarantia2 = TipoGarantia::where('id_tipo_garantia', $this->garantiaC5_2($id, $idc))->firstOrFail()->tipo_garantia;
        }
        //error de registro vacio de garantia end

        $c5_sum_eval_1 = PuntajeRepository::garantias($this->garantiaC5_1($id, $idc));
        $c5_sum_eval_2 = PuntajeRepository::garantias($this->garantiaC5_2($id, $idc));
        $total_c5 = $c5_sum_eval_1 + $c5_sum_eval_2;
        $contador_c5 = ContadorC5Repository::contador_c5($this->garantiaC5_1($id, $idc), $this->garantiaC5_2($id, $idc));
...
//tipo de riesgo crediticio
        ...
        return view('oficial.scor.scor')
            ...
            //Experiencia crediticia
            ->with('experiencia_cre_dias', $this->experienciaCrediticia($id, $idc))
            ->with('penultima_experiencia_cre_dias', $this->penultima_experienciaCrediticia($id, $idc))
            ->with('ppenultima_experiencia_cre_dias', PuntajeRepository::experiencia_cre_ultimo($this->penultima_experienciaCrediticia($id, $idc)))
            ->with('pexperiencia_cre_dias', PuntajeRepository::experiencia_cre_ultimo($this->experienciaCrediticia($id, $idc)))
            ...
            //CALCULO DE C3
            ->with('covertura', round($this->coverturaCuotaIngreso($id, $idc) * 100) / 100)
            ->with('gastos_anterior', round($this->gastosIngresosAnterior($id,$idc) * 100) / 100)
            ->with('gastos_actual', round($this->gastosIngresosActual($id,$idc) * 100) / 100)
            ->with('contador_c3', $contador_c3)
            //c3 evaluado
            ->with('covertura_eval', PuntajeRepository::cobertura_cuota($this->coverturaCuotaIngreso($id, $idc)))
            ->with('gastos_anterior_eval', PuntajeRepository::gasto_anterior($this->gastosIngresosAnterior($id,$idc)))
            ->with('gastos_actual_eval', PuntajeRepository::gasto_actual($this->gastosIngresosActual($id,$idc)))
...
//FUNCIONES DE C1
    ...
    {
        //Datos economicos Depenciente
        $if_exist = DatosEmpresa::where('id_persona', $id)->exists();
        if ($if_exist) {
            $now = Carbon::now();
    ...
    public function experienciaCrediticia($id, $idc)
    {
    ...
    public function penultima_experienciaCrediticia($id, $idc)
    {
        $if_exist = ReporteBuro::where('id_persona', $id)->where('id_credito', $idc)->count();

        if ($if_exist >= 2) {
            $reporte = ReporteBuro::where('id_persona', $id)->where('id_credito', $idc)->get()->last();
...
//END FUNCIONES C1
    ...
    public function total_activos($id, $idc)
    {
        $if_exist_caja = EfectivoCaja::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_caja > 0) {
            $total_efectivo = DB::table('efectivos_caja')->where('id_persona', $id)->where('id_credito', $idc)->sum('caja');
        } else {
            $total_efectivo = 0;
        }

        $if_exist_otro = OtroActivo::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_otro > 0) {
            $total_otro = DB::table('otros_activos')->where('id_persona', $id)->where('id_credito', $idc)->sum('total');
        } else {
            $total_otro = 0;
        }

        $if_exist = DepositoBancario::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist > 0) {
        ...
        $if_exist_d = CuentasDocumentosCobrar::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_d > 0) {
        ...
        $if_exist_i = InversionesFinancieras::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_i > 0) {
        ...
        $if_exist_ma = MaquinariaEquipo::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_ma) {
        ...
        $if_exist_in = InventarioMercaderia::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_in > 0) {
        ...
        $if_exist_pro = Inmueble::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_pro > 0) {
        ...
        $if_exist_ve = Vehiculo::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_ve > 0) {
            $total_vehiculos = DB::table('vehiculo')->where('id_persona', $id)->where('id_credito', $idc)->sum('valor');
        } else {
            $total_vehiculos = 0;
        }
        ...
        if ($if_exist_bienes > 0) {
            $total_bienes = DB::table('bienes_hogar')->where('id_persona', $id)->where('id_credito', $idc)->sum('valor');
        } else {
            $total_bienes = 0;
        }
...
//sumatoria del total de pasivos
    ...
    public function total_pasivos($id, $idc)
    {
        $if_exist = PrestamoBancario::where('id_persona', $id)->where('id_credito', $idc)->count();
        ...
        $if_exist_1 = CuentasPorPagar::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_1 > 0) {
...
//Endeudamiento Actual
    public function endeudamientoActual($id, $idc)
    {
        if ($this->total_activos($id, $idc) == 0) {
            return 0;
        } else {
    ...
    public function endeudamientoConEsteCredito($id, $idc)
    {
        if ($this->total_activos($id, $idc) == 0) {
            ...
            $monto_solicitado = Credito::where('id_credito', $idc)->firstOrFail()->monto_solicitado;
            $tipo_moneda = Credito::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->id_tipo_moneda;
            if ($tipo_moneda == 2) {
                $icredito = Credito::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->id_credito;
                $e_cambio = TipoCambio::where('id_credito', $icredito)->count();
                if ($e_cambio > 0) {
                    $tipo_cambio = TipoCambio::where('id_credito', $icredito)->firstOrFail()->cambio;
                    $endeudamiento_con_este_credito = (($this->total_pasivos($id, $idc) + ($monto_solicitado * $tipo_cambio)) / $this->total_activos($id, $idc)) * 100;
                    return $endeudamiento_con_este_credito;
                }
            } else {
                $endeudamiento_con_este_credito = (($this->total_pasivos($id, $idc) + $monto_solicitado) / $this->total_activos($id, $idc)) * 100;
                return $endeudamiento_con_este_credito;
            }
    ...
    public function totalIngresos($id, $idc)
    {
        $existe_ingreso = IngresoMensual::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($existe_ingreso) {
            $total_ingreso_mensual_solicitante = DB::table('ingreso_mensual')->where('id_persona', $id)->where('id_credito', $idc)->avg('total_ingreso');
    ...
    public function vPrecioTotal($id, $idc)
    {
        $if_exist_registros = VentaComercializacionProducto::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_registros) {
            $total_venta_comercializacion = DB::table('venta_comercializacion_productos')->where('id_persona', $id)->where('id_credito', $idc)->sum('v_precio_total');
    ...
    public function cCostoTotal($id, $idc)
    {
        $if_exist_registros = VentaComercializacionProducto::where('id_persona', $id)->where('id_credito', $idc)->count();
        if ($if_exist_registros >= 1) {
            $total_venta_comercializacion = DB::table('venta_comercializacion_productos')->where('id_persona', $id)->where('id_credito', $idc)->sum('c_costo_total');
            return $total_venta_comercializacion;
        } else {
            return 0;
        }
    ...
    public function manoObra($id, $idc)
    {
        $if_exist_registros = VentaComercializacionProducto::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_registros) {
    ...
    public function gastosOperativos($id,$idc)
    {
        $if_exist_registros = GastosOperativosComercializacion::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_registros) {
...
//funcion del calculo de la utilidad para la capacidad de pago
    public function utilidadCapacidadPago($id,$idc)
    {
        $if_exist_registros = CapacidadPago::where('id_persona', $id)->where('id_credito', $idc)->count();
        ...
        if ($if_exist_registros >= 1) {
            ...
            $amortizacion = CapacidadPago::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->amortizacion_coop_san_martin;
            $importe_ultimo_pago = PrestamoBancario::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->importe_ultimo_pago;
            $total_cuentas_por_pagar = $this->cuentasPorPagar($id,$idc);
            $total_gastos_familiares = $this->gastosFamiliares($id,$idc);
...
//funciones para calcular total egresos
    public function cuentasPorPagar($id,$idc)
    {
        $if_exist_registros = CuentasPorPagar::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_registros) {
    ...
    public function gastosFamiliares($id,$idc)
    {
        $if_exist_registros = GastosFamiliares::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_registros) {
    ...
    public function margenAhorro($id, $idc)
    {
        $margenAhorro = $this->utilidadOperativa($id, $idc) - $this->utilidadCapacidadPago($id,$idc);
        return $margenAhorro;
    }

    public function utilidadOperativa($id, $idc)
    {
        $ingresos = ($this->totalIngresos($id, $idc) + $this->vPrecioTotal($id, $idc)) - ($this->cCostoTotal($id, $idc) + $this->manoObra($id, $idc) + $this->gastosOperativos($id,$idc));
        return $ingresos;
    }

//calculo de cobertura cuota ingreso
    public function coverturaCuotaIngreso($id, $idc)
    {
        $if_exist_cap = CapacidadPago::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_cap) {
            $amortizacion = CapacidadPago::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->amortizacion_coop_san_martin;
            ...
            $total_gastos_familiares = $this->gastosFamiliares($id,$idc);
            //calculo del  total de egresos
            $total_egresos = $total_cuentas_por_pagar + $total_gastos_familiares + $this->amortizacionPrestamo($id, $idc);
    ...
    public function amortizacionPrestamo($id, $idc)
    {
        $suma_amortizacion = DB::table('prestamo_bancario')->where('id_persona', $id)->where('id_credito', $idc)->sum('importe_ultimo_pago');
        return $suma_amortizacion;
    }

    public function gastosIngresosAnterior($id, $idc)
    {
        if ($this->utilidadOperativa($id, $idc) == 0) {
            return 0;
        } else {
            $total_cuentas_por_pagar = $this->cuentasPorPagar($id,$idc);
            $total_gastos_familiares = $this->gastosFamiliares($id,$idc);
            ...
            $total_egresos = $total_cuentas_por_pagar + $total_gastos_familiares + $this->amortizacionPrestamo($id,$idc);
            //calculo de covertura
            $gastos_ingresos_anterior = $total_egresos / $this->utilidadOperativa($id, $idc);
    ...
    public function gastosIngresosActual($id, $idc)
    {
        if ($this->utilidadOperativa($id, $idc) == 0) {
            return 0;
        } else {
            $if_exist_registros = CapacidadPago::where('id_persona', $id)->where('id_credito', $idc)->exists();
            if ($if_exist_registros) {
                $amortizacion = CapacidadPago::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->amortizacion_coop_san_martin;
            } else {
                $amortizacion = 0;
            ...
            $total_gastos_familiares = $this->gastosFamiliares($id,$idc);
            //calculo del  total de egresos
            $total_egresos = $amortizacion + $total_cuentas_por_pagar + $total_gastos_familiares + $this->amortizacionPrestamo($id, $idc);
            //calculo de covertura
            $gastos_ingresos_actual = $total_egresos / $this->utilidadOperativa($id, $idc); // $gastos_ingresos_actual=3384/6627;
...
//C4 ---------------------------------------- Begin functions
    public function ingresos_variables_mensuales($id, $idc)
    {
        $existe_venta = VentaComercializacionProducto::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($existe_venta) {
            $precio_total = DB::table('venta_comercializacion_productos')->where('id_persona', $id)->where('id_credito', $idc)->sum('v_precio_total');
            $compras_precio_total = DB::table('venta_comercializacion_productos')->where('id_persona', $id)->where('id_credito', $idc)->sum('c_costo_total');
            $suma_total = $precio_total - ($compras_precio_total + $this->manoObra($id, $idc) + $this->gastosOperativos($id,$idc));
            return $suma_total;
        } else {
            return 0;
        }
...
//Begin c5
    public function garantiaC5_1($id, $idc)
    {
        $if_exist_credito = Credito::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_credito) {
            $idCredito = Credito::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->id_credito;
    ...
    public function garantiaC5_2($id, $idc)
    {
        $if_exist_credito = Credito::where('id_persona', $id)->where('id_credito', $idc)->exists();
        if ($if_exist_credito) {
            $idCredito = Credito::where('id_persona', $id)->where('id_credito', $idc)->firstOrFail()->id_credito;
