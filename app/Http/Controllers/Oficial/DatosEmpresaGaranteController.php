...
class DatosEmpresaGaranteController extends Controller
{
...
public function index(Request $request)
{
  ...
  if(session('id_persona_garante')==null)
  {
   ...
   return redirect('oficial/garante/');
    ...
    else
    {
      ...
      ->get();
      return view('oficial.a_garantes.datos_empresa.index')->with(compact('datos')); 
    }
  ...
  {    
    ...
    {
      $if_exist=DatosEmpresa::where('id_persona',session('id_persona_garante'))->count();
      if($if_exist>3)
      {
        alert()->info('Info','Ya registro los Datos de la Empresa.')->showConfirmButton();
        return redirect('oficial/a_garantes/datos_empresa/');
      }
      else
      {
        $afp=Afp::all();
        $tipo_contrato=TipoContrato::all();
        return view('oficial.a_garantes.datos_empresa.create')
  ...
  public function store(DatosEmpresaFormRequest $request)
  {
   ...
   $datos->id_persona=$this->id_persona_garante;
 $datos->save(); //metodo se encarga de ejecutar un insert sobre la tabla
 $notification= 'Exelente los datos se han guardado correctamente'; 
 return redirect('oficial/a_garantes/datos_empresa')->with(compact('notification'));
}
...
public function edit($id)
{
 ...
 $tipo=TipoContrato::All();
      return view('oficial.a_garantes.datos_empresa.edit')->with(compact('datos','afp','tipo')); //formulario de registro
    }
