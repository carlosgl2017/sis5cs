...
class ManoObraMensualGaranteController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (session('id_persona_garante') == null) {
            ...
            $mano = ManoObraMensual::where('id_persona', session('id_persona_garante'))->where('id_credito', session('id_credito'))->get();
            $total_mano_obra = DB::table('mano_obra_mensual')->where('id_persona', session('id_persona_garante'))->where('id_credito', session('id_credito'))->sum('total_mano_obra');
