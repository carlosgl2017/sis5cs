...
class UserController extends Controller
{
    ...
    public function index()
    {
      ...
      ->get();
      return view('cliente.user.index',compact('users'));
    }
  ...
  return view('cliente.user.edit')->with(compact('users')); //formulario de registro
}
public function edit_photo()
{
  $id=Auth::user()->id_users;
  $users=User::find($id);
  return view('cliente.user.edit_photo')->with(compact('users')); //formulario de registro
}
...
public function update_datos(UserFormRequest $request,$id)
{
 ...
 return redirect('cliente/user/')->with(compact('notification'));
}
